...
const char ADDR[] = { A8,A9,A10,A11,A12,A13,A14,A15,53,52,51,50,49,48,47,46,45,44,43,42,41,40,39,38 };
const char DATA[] = { 37,36,35,34,33,32,31,30,29,28,27,26,25,24,23,22 };
...
#define MWTC			A4	//Memory write (assuming RAM write for now)
#define MRDC			A3	//Memory read
#define IOWC			A5	//IO write
#define IORC			A6	//IO read
//#define PPI1_CS			7
//#define PPI2_CS			8
//#define IRQ_CTLR_CS		9
//#define MATH_CO_EN		13
#define	DTR				A1
#define	DEN				A2
#define ALE				A7
...
void onClock()
{
  ...
  int FullAddr = addressTop8 + address;

  //Serial.print(FullAddr);
...
// 
  ...
  sprintf(output, "  0x:%02x%04x:%04x  %s  %s %s %s  CYCLE:%s%s%s%s  BHE:%s MWTC:%s MRDC:%s IORC:%s IOWC:%s ALE:%s DEN:%s DTR:%s  debug:%i", addressTop8, address, data, currentInstruction.c_str(), 
		digitalRead(RAM_OE) ? "----" : "RAMR", digitalRead(MWTC) ? "----" : "RAMW", digitalRead(ROM_OE) ? "----" : "ROMR",
		CYCLE_CODINTA ? "1" : "0", CYCLE_MIO ? "1" : "0", CYCLE_S1 ? "1" : "0", CYCLE_S0 ? "1" : "0", BUS_HIGH_ENABLE ? "1" : "0",
		digitalRead(MWTC) ? "1" : "0", digitalRead(MRDC) ? "1" : "0", digitalRead(IORC) ? "1" : "0", digitalRead(IOWC) ? "1" : "0",
		digitalRead(ALE) ? "1" : "0", digitalRead(DEN) ? "1" : "0", digitalRead(DTR) ? "1" : "0", InstructionBytesRemaining);
